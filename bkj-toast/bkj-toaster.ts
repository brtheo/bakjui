import './bkj-toast.js'
import {BkjToast} from "./bkj-toast.js"
...
export default class BkjToaster {
    ...
    public burn(templateKey: ToastTemplateKey, data: ToastTemplateData ): void {
        ...
        this.queue.push([`<bkj-toast type="${type}" class="${templateKey}">${template(data)}</bkj-toast>`,animation])
     ...
     * @param toast Object containing the template function, the type and the animations keyframes and options to use corresponding with the templateKey provided.
     */
    public registerTemplate(templateKey: ToastTemplateKey, toast: Toast): void {
     ...
     */
    public registerToastEvent(callback: ($burntToast: BkjToast) => void): void{
        document.addEventListener('toastBurning', () => {
            try {
                const $burntToast = document.querySelector('bkj-toast')
                return callback($burntToast as BkjToast)
            } catch{}
        })
    }
}